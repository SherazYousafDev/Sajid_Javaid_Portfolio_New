...
const Sidebar = () => {
  ...
  const navItems = useMemo(
    () => [
      ...
      { label: "Skills", icon: <Code size={18} />, href: "#skills", id: "skills" },
      { label: "Projects", icon: <Award size={18} />, href: "#projects", id: "projects" },
      { label: "Experience", icon: <Briefcase size={18} />, href: "#experience", id: "experience" },
      { label: "Certificates", icon: <Award size={18} />, href: "#certificates", id: "certificates" },
      { label: "Education", icon: <School size={18} />, href: "#education", id: "education" },
      { label: "Contact", icon: <MessageSquare size={18} />, href: "#contact", id: "contact" },
    ],
    []
  );
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Reduced offset to 100px so it only switches when the section header is close to the top
      const scrollPosition = window.scrollY + 100;

      // 1. Force "About" to stay active when at the top of the page
      ...
      // 3. Special case: If at the bottom, highlight Contact
  ...
  const ContactInfo = ({ icon: Icon, text, href }) => (
    <div className="flex items-center gap-3 text-secondary hover:text-primary transition-colors cursor-pointer group/info">
      <Icon size={14} className="shrink-0 group-hover/info:scale-110 transition-transform" />
      {href ? (
        <a href={href} className="text-[10px] font-bold truncate tracking-wider">{text}</a>
      ) : (
        <span className="text-[10px] font-bold truncate tracking-wider">{text}</span>
      )}
    </div>
  );
    ...
    <>
      ...
      <aside
        className={`
          ...
          ${isOpen ? "translate-x-0 shadow-2xl shadow-primary/5" : "-translate-x-full lg:translate-x-0"}
        `}
      >
        <div className="mb-8 group">
          <div className="relative inline-block mb-4">
            <div className="hidden md:block h-16 w-16 rounded-2xl overflow-hidden border-2 border-primary/20 shadow-lg shadow-primary/5 group-hover:rotate-3 transition-transform duration-300 ">
              <img src="/profile.webp" alt="Sajid Javaid" className="w-full h-full object-cover transition-all duration-500" />
            </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-secondary text-[9px] font-black uppercase tracking-[0.2em]">Fintech Expert</span>
          </div>
        </div>
        ...
        <nav className="flex-1 flex flex-col space-y-1 overflow-visible">
          {navItems.map((item) => (
            <a
              ...
              `}
            >
              <span className={`transition-colors duration-300 ${activeSection === item.id ? "text-primary" : "group-hover:text-text"}`}>
                {item.icon}
              </span>
              <span className="font-bold text-[11px] tracking-wide uppercase">{item.label}</span>
              <div
                className={`ml-auto w-1.5 h-1.5 rounded-full transition-all duration-500 ${
                  activeSection === item.id ? "bg-primary shadow-[0_0_12px_var(--color-primary)] scale-100 opacity-100" : "scale-0 opacity-0"
                }`}
              ></div>
            </a>
          ))}
        ...
        <div className="mt-6 p-4 bg-card/50 rounded-2xl border border-secondary/20 space-y-3">
          <ContactInfo icon={Mail} text="sajid.javaid63@gmail.com" href="mailto:sajid.javaid63@gmail.com" />
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-bg/60 backdrop-blur-sm z-40 lg:hidden transition-all duration-300" onClick={() => setIsOpen(false)} />
      )}
    </>
  );
};
